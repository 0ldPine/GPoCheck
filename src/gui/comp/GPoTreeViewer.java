...
public class GPoTreeViewer extends JComponent {
	...
	public static class VariableExtentProvide implements NodeExtentProvider<Tree> {
		GPoTreeViewer viewer;

		public VariableExtentProvide(GPoTreeViewer viewer) {
			this.viewer = viewer;
		}
	...
	protected Color boxColor = null; // set to a color to make it draw
										// background
	...
	public GPoTreeViewer(@Nullable List<String> ruleNames, Tree tree) {
	...
	protected void paintBox(Graphics g, Tree tree) {
		...
		if (isHighlighted(tree) || boxColor != null || tree instanceof ErrorNode) {
	...
	@NotNull
	protected static JDialog showInDialog(final GPoTreeViewer viewer) {
	...
	private static void generatePNGFile(GPoTreeViewer viewer, JDialog dialog) {
		...
		try {
			...
			if (returnValue == JFileChooser.APPROVE_OPTION) {
				...
				try {
					// Try to open the parent folder using the OS' native file
					// manager.
					Desktop.getDesktop().open(pngFile.getParentFile());
				} catch (Exception ex) {
					// We could not launch the file manager: just show a popup
					// that we
	...
	private static void fillTree(TreeNodeWrapper node, Tree tree, GPoTreeViewer viewer) {
	...
	public JPanel open() {
		...
		final GPoTreeViewer viewer = this;
		viewer.setScale(1.5);
		...
		// Wrap viewer in scroll pane
		JScrollPane scrollPane = new JScrollPane(viewer);
		// Make the scrollpane (containing the viewer) the
		// center component
		contentPane.add(scrollPane, BorderLayout.CENTER);
		...
		// Add a JTree representing the parser tree of the
		// input.
		JPanel treePanel = new JPanel(new BorderLayout(5, 5));

		// An "empty" icon that will be used for the JTree's
		// nodes.
		Icon empty = new EmptyIcon();
	...
	public void setTree(Tree root) {
		if (root != null) {
			...
			this.treeLayout = new TreeLayout<Tree>(new TreeLayoutAdaptor(root), new GPoTreeViewer.VariableExtentProvide(this), new DefaultConfiguration<Tree>(gapBetweenLevels, gapBetweenNodes), useIdentity);
	...
	private static class TreeNodeWrapper extends DefaultMutableTreeNode {
		...
		private static final long serialVersionUID = 1L;
		final GPoTreeViewer viewer;

		TreeNodeWrapper(Tree tree, GPoTreeViewer viewer) {
			super(tree);
			this.viewer = viewer;
		}
