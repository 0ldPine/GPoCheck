...
package engine.sparse.csparsej.tdouble;
...
public class Dcs_print {
	...
	public static boolean cs_print(Dcs A, boolean brief) {
		...
		nzmax = A.nzmax;
		nz = A.nz;
		// System.out.print(String.format("CSparseJ Version %d.%d.%d, %s.  %s\n", Dcs_common.CS_VER, Dcs_common.CS_SUBVER, Dcs_common.CS_SUBSUB, Dcs_common.CS_DATE, Dcs_common.CS_COPYRIGHT));
		if (nz < 0) {
			System.out.print(String.format("%d-by-%d, nzmax: %d nnz: %d, 1-norm: %g\n", m, n, nzmax, Ap[n], Dcs_norm.cs_norm(A)));
			for (j = 0; j < n; j++) {
