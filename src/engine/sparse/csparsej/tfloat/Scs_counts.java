...
public class Scs_counts {
    ...
    private static int HEAS(int k, int j, int[] head, int head_offset, boolean ata) {
    ...
    private static int[] init_ata(Scs AT, int[] post, int[] w) {
    ...
    public static int[] cs_counts(Scs A, int[] parent, int[] post, boolean ata) {
        ...
        for (k = 0; k < s; k++)
            w[k] = -1; /* clear workspace w [0.f.s-1] */
        ...
        for (k = 0; k < n; k++) {
            ...
            for (J = HEAS(k, j, head, head_offset, ata); J != -1; J = NEXT(J, next, next_offset, ata)) /* J=j for LL'=A case */
            {
                for (p = ATp[J]; p < ATp[J + 1]; p++) {
                    i = ATi[p];
                    q = Scs_leaf.cs_leaf(i, j, first, first_offset, maxfirst, maxfirst_offset, prevleaf,
