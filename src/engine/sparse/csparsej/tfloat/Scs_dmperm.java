...
public class Scs_dmperm {
    ...
    private static boolean cs_bfs(Scs A, int n, int[] wi, int[] wj, int[] queue, int[] imatch, int imatch_offset,
        ...
        C = (mark == 1) ? A : Scs_transpose.cs_transpose(A, false);
        if (C == null)
    ...
    /**
     * Compute coarse and then fine Sulmage-Mendelsohn decompositionm. seed
    ...
    public static Scsd cs_dmperm(Scs A, int seed) {
        ...
        m = A.m;
        n = A.n;
        S = Scs_util.cs_dalloc(m, n); /* allocate result */
        if (S == null)
            return (null);
        p = S.p;
        q = S.q;
        ...
        cc = S.cc;
        rr = S.rr;
        jmatch = Scs_maxtrans.cs_maxtrans(A, seed); /* max transversal */
        ...
        jmatch = null;
        /* --- Fine decomposition ----------------------------------------------- */
        pinv = Scs_pinv.cs_pinv(p, m); /* pinv=p' */
        if (pinv == null)
            return (null);
        C = Scs_permute.cs_permute(A, pinv, q, false);/* C=A(p,q) (it will hold A(R2,C2)) */
        pinv = null;
        if (C == null)
            return (null);
        Cp = C.p;
        ...
        {
            Scs_fkeep.cs_fkeep(C, new Cs_rprune(), rr);
            cnz = Cp[nc];
            Ci = C.i;
            if (rr[1] > 0)
                for (k = 0; k < cnz; k++)
        ...
        C.m = nc;
        scc = Scs_scc.cs_scc(C); /* find strongly connected components of C*/
        if (scc == null)
            return (null);
